...
export class Switch extends Component {
  ...
  render() {
    ...
    const interpolatedActiveTextOpacity = transformSwitch.interpolate({
      inputRange: [-circleSize / switchRightPx, circleSize / switchRightPx],
      outputRange: [-1, 1]
    });

    const interpolatedInactiveTextOpacity = transformSwitch.interpolate({
      inputRange: [-circleSize / switchRightPx, circleSize / switchRightPx],
      outputRange: [1, -1]
    });
        ...
        >
          ...
          >
            {value && renderActiveText && (
              <Animated.Text style={[styles.text, styles.paddingRight, activeTextStyle, { opacity: interpolatedActiveTextOpacity }]}>
                {activeText}
              </Animated.Text>
            )}
            ...
            {!value && renderInActiveText && (
              <Animated.Text
                style={[styles.text, styles.paddingLeft, inactiveTextStyle, { opacity: interpolatedInactiveTextOpacity }]}
              >
                {inActiveText}
              </Animated.Text>
            )}
